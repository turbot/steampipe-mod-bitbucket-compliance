with my_repo as (
  ...
  where
    ...
    and kind = 'reset_pullrequest_approvals_on_change'
)
select
  -- Required Columns
  ...
  case
    ...
    r.full_name || ' default branch ' || r.main_branch_name || case when (br.full_name is not null) then ' ' else ' does not ' end || 'dismiss stale reviews.' as reason,
